...
main:		stp x29, x30, [sp, -16]								// Save FP and LR to stack, allocating 16 bytes, pre-increment SP
		...
		movk multiplicand_r, 0xFEFE, lsl 0 						// Initialize "multiplicand" variable with the value -16843010. Place the first half of 0xFEFEFEFE in.
		movk multiplicand_r, 0xFEFE, lsl 16 						// Place remainder of "0xFEFEFEFE" in.
		mov multiplier_r, 70								// Initialize "multiplier" variable with the value 70.
...
loop_1:		AND w27, multiplier_r, 0x1							// Completes an AND operation, placing the result into the W27 register
		cmp w27, 1									// Compare the result of the AND operation to zero
		b.eq add_product
        b next_2									// If the result of AND is equal to zero, this indicates that the IF was false.
