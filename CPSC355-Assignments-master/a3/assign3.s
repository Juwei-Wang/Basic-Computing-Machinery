...
// Note: This section is not necessary, but is used to improve readability of the code. Load and store instructions are still performed on memory from registers.
								...
								// Define the register holding the stack adress for the base of the array.
								// Define the register holding the index i as register w27.
								// Define the register holding the index j as register w26.
...
main:		stp x29, x30, [sp, alloc]!					// Save FP and LR to stack, allocating by the amount "alloc", pre-increment SP. 
		...
		mov x28, fp						// Calculates the base address of the array. 
		add x28, x28, ia_s					// Location of Base of array = (fp + offset from fp to ia_base)
...
// ------------------------------------------------------------------ LOOP 1 : ARRAY INITIALIZATION ----------------------------------------------------------------
		...
		mov w27, 0						// Set index to zero.
		str w27, [fp, i_s]					// Write index i to stack.

		b test_1							// Branch to optimized test_1										

loop_1:									
		ldr w27, [fp, i_s]					// Load the index for i from memory
		...
		str w0, [x28, w27, SXTW 2]				// Store data from w0 (which holds random number) into v[i]. Sign extend it to fit in our 64-bit memory addresses.
	
		ldr w20, [x28, w27, SXTW 2]				// Read number from v[i] and store in temporary register w20
		...
		mov w1, w27						// Move i into x1
		mov w2, w20							// Load v[i] from memory, and put it into register x2
		bl printf							// Use bl (branch to label) to call function printf()

		ldr w27, [fp, i_s]					// Get current value of index i
		add w27, w27, 1					// Increment i by 1.
		str w27, [fp, i_s]					// Store new i in memory.


test_1:		cmp w27, array_size					// If i < 50...
...
// ----------------------------------------------------------------- LOOP 2A --------------------------------------------------------------------------------------
		...
		mov w27, 0						// Set i = 0
		str w27, [fp, i_s]					// Store initial value of i into stack.
	
		b test_2a							// Branch to optimized loop test.
		
loop_2a:	
		mov w20, w27						// Set w20 to i. (min = i)
		str w20, [fp, min_s]						// Store value of min to stack memory.

// ----------------------------------------------------------------- LOOP 2B -------------------------------------------------------------------------------------

		ldr w27, [fp, i_s]					// Load current value of i to register
		mov w26, w27					// Set j = i
		add w26, w26, 1					// j++
		str w26, [fp, j_s]					// Store value of j into stack memory.
...
loop_2b:
		...
		ldr w20, [x28, w19, SXTW 2]				// Load v[min] into w20. SXTW sign extends, and LSL twice, to multiply offset by 4
		ldr w21, [x28, w26, SXTW 2]				// Load v[j] into w21. SXTW sign extends, and LSL twice to multiply offset by 4

		cmp w20, w21							//Compare v[min] to v[j]
		b.lt next							// If v[min] is less than v[j], skip this step.

		str w26, [fp, min_s]					// Store new "min" value in stack memory.

next:		
		add w26, w26, 1					// j++		
		str w26, [fp, j_s]					// Store new value of j to memory.


test_2b:	cmp w26, array_size					// If j < 50...
...
// --------------------------------------------------------------- / END LOOP 2B ---------------------------------------------------------------------------------
		...
		ldr w20, [x28, w19, SXTW 2]				// Load value of v[min] into w20
		ldr w27, [fp, i_s]					// Load value of i from stack memory.
		ldr w21, [x28, w27, SXTW 2]				// Load value of v[i] into w21

		mov w22, w21							// Temp = v[i]
		mov w21, w20							// v[i] = v[min]
		mov w20, w22							// v[min] = Temp

		str w20, [x28, w19, SXTW 2]				// Store v[i] into memory where v[min] was.
		str w21, [x28, w27, SXTW 2]				// Store v[min] into memory where v[i] was.
		

		adrp x0, print_3						// Print out the value to standard output, in the form "v[i] = x"
		add w0, w0, :lo12:print_3					// Add lower-order bits to x0 for print 3
		mov w1, w27						// Move i into w1
		ldr w20, [x28, w27, SXTW 2]				// Load value of v[i] to temporary register w20
		mov w2, w20							// Load v[i] from memory, and put it into register x2
		bl printf							// Use bl (branch to label) to call function printf()


		ldr w27, [fp, i_s]					// Load current value of i
		add w27, w27, 1					// i++
		str w27, [fp, i_s]					// Store new value of i back into stack memory.


test_2a:	cmp w27, array_size					// if i < 50 ...
