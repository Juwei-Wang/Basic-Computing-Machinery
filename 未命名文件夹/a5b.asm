...
main:		stp x29, x30,[sp, -16]!		//Save frame pointer  and link register to stack, allocating bytes, pre-increment SP
			...
			b.eq next					//if so, branch to next
...
next:		mov i_r, 1						//i = 1
			...
			mov w0, mon_r					//set the month as the first arg of function numOfDay
			bl numOfDay						//find the number of days in a particular month
			mov w26, w0						//get the return value
			
			cmp mon_r,1						//month < 1 ?
			b.lt printErr					//if so it is invalid
			cmp mon_r,12					//month > 12 ?
			b.gt printErr					//if so it is invalid
			cmp day_r,1						//day < 1?
			b.lt printErr					//if so it is invalid
			cmp day_r,w26					//day > the number of day a surtain month suppose to have?
			b.le moveon						//if not , the input date is valid , move on
			
printErr:	adrp x0, er2					//set the 1st arg of printf
			add  x0, x0, :lo12:er2			//set teh 1st arg of printf
			bl printf						//call printf
			b endmain						//branch to endmain
			
moveon:
			mov w0, day_r					//set the 1st arg of function whichSuffix 			
			bl whichSuffix					//call whichSuffix
			mov x26, x0 					//contain address of which suffix to print
			
			mov w0, mon_r					//set the 1st arg of function whichSeason
			mov w1, day_r					//set the 2nd arg
			bl whichSeason					//call whichSeason
...
endmain:	ldp x29, x30, [sp], 16              //restore fp and lr from stack, post-increment sp
			ret									//return to the caller	
//function numOfDay
numOfDay:	cmp w0, 2							//compare the input value of month with 2
			b.ne nextif							//if its not 2 branch to nextif
			mov w9, 28							//if so, month is Feb and has 28 days
			b done								//branch to done to end the function
//check if the input month is one of April, June, September and November, that have 30 days
nextif:		cmp w0, 4							//is April?
			b.eq case30							//if so branch to case30
			cmp w0, 6							//June?
			b.eq case30							//if so branch to case30
			cmp w0, 9							//September?
			b.eq case30							//if so branch to case30
			cmp w0, 11							//November?
			b.eq case30							//if so branch to case30
case31:		mov w9, 31							//the remaining untested case31
			b done								//branch to done
case30:		mov w9, 30							//set w9 to 30
done:		mov w0, w9							//set w0 to w9
...
whichSeason:
			stp x29, x30,[sp, alloc]!			//Save frame pointer  and link register to stack, allocating bytes, pre-increment SP
...
loop:		ldr w0,[x29,i_s]					//set the 1st arg of numOfDay
			bl numOfDay							//call numOfDay
			ldr w9,[x29,sum_s]					//get value of sum
			add w9,w9,w0						//added by the return value of numOfDay
...
test:		ldr w9,[x29,i_s]					//get i
			...
			b.lt isSpr							//if less than, check if it is spring
			cmp w9, 263							//Sep. 20 is the 263th day
			b.gt isFal							//if greater than, check if it is fall
			...
			b endsubrout						//branch to endsubrout
			
isSpr:		cmp w9, 80						//< 80?(March 21)
			b.lt caseWin					//if so it is winter
			adrp x10, season_m 				//calculate the base address of the external array season_m
			add x10, x10, :lo12:season_m 	//calculate the base address of the external array season_m
			mov w11, 1						//is spring
...
isFal:		cmp w9, 354						//>354(Dec. 20)?
			b.gt caseWin					//if so it is winter
			adrp x10, season_m 				//calculate the base address of the external array season_m
			add x10, x10, :lo12:season_m 	//calculate the base address of the external array season_m
			mov w11, 3						//is fall
...
caseWin:	adrp x10, season_m 				//calculate the base address of the external array season_m
			add x10, x10, :lo12:season_m 	//calculate the base address of the external array season_m
			mov w11, 0						//is winter
